...
if not required_pred_cols.issubset(preds.columns):
    raise ValueError(f"predictions.csv missing {required_pred_cols - set(preds.columns)}")
...
if not have_true_cols:
    print("True columns not found in predictions.csv – joining from processed parquet...")
    base = pd.read_parquet(PROCESSED_PARQUET, columns=["ticker_id", "time_idx", "target_1d", "target_5d", "target_20d"])
...
def per_target_analysis(df, tgt_name="20d"):
    ...
    d = df[[ "ticker", "time_idx_h1", p50, p05, p95, y, "ticker_x", "ticker_y" ] \
            if "ticker_x" in df.columns or "ticker_y" in df.columns else \
           ["ticker", "time_idx_h1", p50, p05, p95, y]].copy()
    ...
    d["k_ratio"] = d["ae"] / (1.645 * d["s_hat"].replace(0, np.nan))  # used for calibration
    ...
    by_ticker = d.groupby(["ticker", "ticker_symbol"], dropna=False).agg(
        n=("ae", "size"),
        mae=("ae", "mean"),
    ...
    by_date = d.groupby("time_idx_h1").agg(
        n=("ae", "size"),
        mae=("ae", "mean"),
        rmse=("resid", lambda x: np.sqrt((x**2).mean())),
        cover_90=("inside", "mean"),
