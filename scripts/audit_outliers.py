...
def load_data(run_dir: Path, data_parquet: Path, ticker_map_path: Path | None):
    ...
    if (
        "ticker_symbol" not in preds.columns
        and ticker_map_path
        and Path(ticker_map_path).exists()
    ):
        ...
        if set(["ticker_id", "ticker"]).issubset(tmap.columns):
            ...
            preds = preds.merge(
                tmap, left_on="ticker", right_on="ticker_id", how="left"
            )
            preds.rename(
                columns={"ticker": "ticker_symbol_x", "ticker_x": "ticker_symbol"},
                inplace=True,
            )
            # clean columns
            ...
            drop_cols = [
                c
                for c in ["ticker_symbol_x", "ticker_id_y", "ticker_y"]
...
def write_outliers(df: pd.DataFrame, name: str, out_dir: Path, topk: int):
    df = df.copy()
    true_col = (
        f"{name}@h1_true" if f"{name}@h1_true" in df.columns else f"target_{name}"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Audit top residual outliers from predictions.csv"
    )
    ...
    if not names:
        raise RuntimeError(
            "Could not infer targets (columns like '<name>@h1') in predictions.csv"
        )
    ...
    for name in names:
        ...
        if (true_col not in df.columns) and (tgt_col not in df.columns):
            print(
                f"Warning: truth missing for '{name}' (neither '{true_col}' nor '{tgt_col}') – skipping."
            )
            continue
