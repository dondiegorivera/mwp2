...
def recent_median_price_flag(df: pd.DataFrame, price_col: str, lookback_days: int, min_price: float) -> pd.Series:
    ...
    recent = df.loc[is_recent & (df["is_missing"] < 0.5), ["ticker_id", price_col]].copy()
...
def compute_liquidity_flags(df: pd.DataFrame, price_col: str, liq_lookback_days: int,
                            min_nonzero_frac: float, auto_target_tickers: int, min_value20: float):
    ...
    df["value_med20"] = (
        df.groupby("ticker_id", sort=False)["value_traded"]
    ...
    med20_recent = recent.groupby("ticker_id")["value_med20"].median().rename("median_value20_recent")

    # fraction of recent days with nonzero volume (fallback)
    nz_frac = (
        (recent["value_traded"] > 0).groupby(recent["ticker_id"]).mean()
    ...
    if auto_target_tickers and auto_target_tickers > 0:
        ...
        print(f"[auto] median_value20_recent cutoff (top-{auto_target_tickers}): {cutoff:,.0f}")
    ...
    liq["liq_nzfrac_bad"] = liq["nonzero_volume_frac"].fillna(0) < float(min_nonzero_frac)

    return df.merge(liq, on="ticker_id", how="left"), liq

def compute_event_windows_from_raw_prices(df: pd.DataFrame, price_col: str, threshold: float, window: int) -> Tuple[pd.DataFrame, int]:
    """
...
def main():
    ...
    ap.add_argument("--drop-event-windows", action=argparse.BooleanOptionalAction, default=True)
    ...
    id2sym = dict(zip(tmap["ticker_id"].cast(pl.Utf8).to_list(), tmap["ticker"].to_list()))
    ...
    non_miss = (pd.to_numeric(df.get("is_missing", 0.0), errors="coerce").fillna(1.0) < 0.5)
    hist_counts = df.loc[non_miss].groupby("ticker_id")["time_idx"].size().rename("hist_non_missing")
    ...
    price_bad = recent_median_price_flag(df, price_col, args.price_lookback_days, float(args.min_price))

    # --- liquidity (recent window) & fallback for no-volume feeds ---
    df, liq = compute_liquidity_flags(
        df, price_col, args.liq_lookback_days,
        float(args.min_nonzero_volume_frac),
        int(args.auto_target_tickers), float(args.min_median_value20)
    )
    ...
    if args.drop_event_windows:
        ...
        df_kept, dropped_events = compute_event_windows_from_raw_prices(
            df_kept, price_col=price_col,
            threshold=float(args.event_threshold), window=int(args.event_window)
        )
        after = len(df_kept)
